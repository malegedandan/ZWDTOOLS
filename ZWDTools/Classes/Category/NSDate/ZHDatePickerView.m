...
- (void)doneClick
{
    ...
    [dateFormmater2 setDateFormat:@"yyyyMMdd"];
    NSString *dateStr = [dateFormmater2 stringFromDate:select];
    
    self.dateBlock(resultString,dateStr);
    if (self.delegate && [self.delegate respondsToSelector:@selector(pickerView:didSelectDateString:andDateStr:)])
    {
        [self.delegate pickerView:self didSelectDateString:resultString andDateStr:dateStr];
    }
